...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%STARTUP%\EduPal Attention Tracker.lnk'); $s.TargetPath = '%CD%\start_attention_server.bat'; $s.WorkingDirectory = '%CD%'; $s.Save()"
