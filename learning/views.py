...
from .models import (
    ...
    Checkpoint, StudentCheckpointAnswer, VRLesson,
)
...
def vr_lesson_setup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                default_vr_url = 'https://aistudio.google.com/apps/ea0032ea-b331-4cc8-a5fd-e59bbce58fbe?fullscreenApplet=true&showPreview=true&showAssistant=true'
                if not vr_url:
    ...
    design_platform_url = 'https://aistudio.google.com/apps/ea0032ea-b331-4cc8-a5fd-e59bbce58fbe?fullscreenApplet=true&showPreview=true&showAssistant=true'
...
def get_subjects_for_class(request):
    ...
    try:
        # احصل على المواد المرتبطة بهذا الصف والمعلم
        subjects = Subject.objects.filter(
            teacherid=teacher,
        ...
        logger.info(f"Found {len(subjects_list)} subjects for classroom {classroom_id} and teacher {teacher.userid.username}")
        ...
        logger.exception(f"Error getting subjects for classroom {classroom_id}: {e}")
        return JsonResponse({'success': False, 'error': f'خطأ: {str(e)}'}, status=500)
...
def get_lessons_for_subject(request):
    ...
    try:
        ...
        logger.info(f"Found {len(lessons_list)} lessons for subject {subject_id} and teacher {teacher.userid.username}")
